...
<section class="section">
    <script>
        function detail($id_dewasa) {
            $.ajax({
                url: "<?= site_url("/admin/bayar_dewasa") ?>/" + $id_dewasa,
                type: "get",
                success: function(hasil) {
                    var $obj = $.parseJSON(hasil);
                    if ($obj.id_dewasa != '') {
                        $('#IDdewasa').val($obj.id_dewasa);
                ...
                <div class="card-content">
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <h4 class="card-title">List Bayar Kaos Dewasa</h4>
                            <table class="table mb-0">
                                ...
                                <tbody class="bg-dark text-white text-center">
                                    ...
                                    <?php foreach ($dewasa as $bayardw) :
                                        ...
                                        if ($bayardw['jumlah'] >= $bayardw['harga'] || $bayardw['jumlah'] == $bayardw['harga']) {
                                            $status = 'LUNAS';
                                        } else {
                                            $status = 'BELUM LUNAS';
                                        };
                                    ?>
                                        ...
                                        <tr>
                                            ...
                                            <td><?= $bayardw['ukuran'] ?></td>
                                            <td><?= $bayardw['model'] ?></td>
                                            <td>Rp. <?= $bayardw['jumlah'] ?></td>
                                            <td><?= $status ?></td>

                                            <td>
                                                <button type="button" class="btn btn-danger btn-sm" data-bs-toggle="modal" data-bs-target="#exampleModal" onclick="detail(<?= $bayardw['id_dewasa']; ?>)">
                                                    Pilih
                                                </button>
                                            </td>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header bg-danger text-white">
                        <h5 class="modal-title text-white" id="exampleModalLabel">Form Bayar Kaos Dewasa</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body bg-secondary text-white">
                        <form action="bayar/billing_d" method="post">
                            <?= csrf_field() ?>
                            <div class="mb-3">
                                <label for="Namalengkap" class="form-label  fw-bold">ID USER</label>
                                <input type="text" class="form-control text-danger fw-bold" id="IDdewasa" name="id_dewasa" aria-describedby="emailHelp" readonly>
                            </div>
                            <div class="mb-3">
                            ...
                            <?php $isInvalidtanggal = (session()->getFlashdata('errTanggal')) ? 'is-invalid' : '';
                            ?>
                            <div class="mb-3">
                                <label for="Model" class="form-label  fw-bold">Tanggal Bayar</label>
                                <input type="date" class="form-control text-danger fw-bold  <?= $isInvalidtanggal ?>" id="Tanggal" name="tgl_transaksi" aria-describedby="emailHelp" required>
                                <?php
                            ...
                            <div class="mb-3">
                                ...
                                <input type="number" class="form-control text-danger fw-bold" id="Model" name="jml_transaksi" aria-describedby="emailHelp" required>
                            </div>
                            <div class="mb-3">
